...
def run_multiple_groups(BusServo, action_group):
    for group in action_group:
      ...
      max_time = max(int(action['time']) for action in group)  # Convert to int if necessary (convert float to int)
...
def move_object_to_destination(b, action_group_to, action_group_from, object_label, destination_label):
    print("move object {} to destination {}".format(object_label, destination_label))#string (format)
...
if __name__ == '__main__':
    ...
    while True:
      ...
      time.sleep_ms(1000)#micropython sleep function
