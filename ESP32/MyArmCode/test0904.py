...
class ArmController:
    ...
    # 1. 將 object_coordinates 的 x, y 值做轉換成 converted_x, converted_y
        converted_x, converted_y = self.convert_input_to_arm_coordinates(object_coordinates[0], object_coordinates[1])
        
        # 2. 將 object_coordinates 的 z 值 default 設為 90
        adjusted_z = 90
        print(f"Initial Z value: {adjusted_z}")
        
        while adjusted_z >= 30:
            # 3. 丟入 ArmIK.CalcAngle() 得到 angle 值
            ...
            # 6. 閉爪 (設置夾爪閉合)
            self.bus_servo.run(1, 700, 500)
            print(f"Attempting to grab at Z: {adjusted_z}")
            
            # 7. 判斷夾爪位置，成功夾取或調整 Z 軸再嘗試
            timeout = time.time() + 5  # 設置5秒超時
            while time.time() < timeout:
                current_position = self.bus_servo.get_position(1)
                print(f"Current gripper position: {current_position}")
                
                if current_position >= 650:
                    ...
                    if adjusted_z < 30:
                        ...
                        return False
                    self.bus_servo.run(1, 200, 500)  # 再次開爪
                    time.sleep(0.5)
                    self.bus_servo.run(1, 700, 500)  # 再次閉爪
                    time.sleep(0.5)
                elif 590 <= current_position <= 630:
                    print("Successful grab detected.")
                    return True
                
                time.sleep(0.1)
            
            # 如果超時沒有抓取成功，減少 Z 軸再嘗試
            print("Retrying with lower Z-axis...")
        
        # 8. 若 Z = 30 則達到臨界值，停止操作
        print("Failed to grab object, reached Z-axis limit.")
        return False
