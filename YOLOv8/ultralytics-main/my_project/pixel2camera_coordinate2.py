...
def main():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            results1 = model1(frame, show=False,conf=0.5)#number detect
            results2 = model2(frame, show=False,conf=0.5)#cube detect
                ...
                for result2 in results2:
                    ...
                    for box1, confidence1, class_id1 in zip(boxes1, confidences1, class_ids1):
                        for box2, confidence2, class_id2 in zip(boxes2, confidences2, class_ids2):
                            ...
                            # print(f"Detected object: Label {label1}, Confidence {confidence1:.2f}")
                            # print(f"Center coordinates (pixels): ({x_center1:.2f}, {y_center1:.2f})")
                            ...
                            print(f"Detected object: Label {label1}, Confidence {confidence1:.2f}")
                            print(f"Center coordinates (pixels): ({x_center1:.2f}, {y_center1:.2f})")
                            print(f"Center coordinates (camera): ({camera_coordinates1[0]:.2f}, {camera_coordinates1[1]:.2f}, {camera_coordinates1[2]:.2f})")
                            print(f"Corrected number Center coordinates (camera): ({x_corrected1:.2f}, {y_corrected1:.2f}, {z_corrected1:.2f})")
                            print()
                            print(f"Detected object: Label {label2}, Confidence {confidence2:.2f}")
                            print(f"Center coordinates (pixels): ({x_center2:.2f}, {y_center2:.2f})")
                            print(f"Center coordinates (camera): ({camera_coordinates2[0]:.2f}, {camera_coordinates2[1]:.2f}, {camera_coordinates2[2]:.2f})")
                            print(f"Corrected cube Center coordinates (camera): ({x_corrected2:.2f}, {y_corrected2:.2f}, {z_corrected2:.2f})")
                            
                            cv2.rectangle(frame, (int(box1[0]), int(box1[1])), (int(box1[2]), int(box1[3])), (255, 255, 0), 2)
                            cv2.circle(frame, (int(x_center1), int(y_center1)), 5, (0, 0, 255), -1)
                            cv2.putText(frame, label1, (int(box1[0]), int(box1[1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)

                            cv2.rectangle(frame, (int(box2[0]), int(box2[1])), (int(box2[2]), int(box2[3])), (255, 255, 0), 2)
                            cv2.circle(frame, (int(x_center2), int(y_center2)), 5, (0, 0, 255), -1)
                            cv2.putText(frame, label2, (int(box2[0]), int(box2[1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
            screen_width = 1500
            screen_height = 800
            window_width = screen_width // 2
            window_height = screen_height // 2
