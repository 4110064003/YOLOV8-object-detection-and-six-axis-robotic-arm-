...
def process_frame(frame):
    ...
    for box, confidence, label in zip(selected_boxes1, selected_confidences1, labels1):
        ...
        object_list[label] = f"{label},({int(camera_coordinates[0])},{int(camera_coordinates[1])},{50})"
    ...
    for box, confidence, label in zip(selected_boxes2, selected_confidences2, labels2):
        ...
        camera_coordinates = pixel_to_camera_coordinates(x_center, y_center)
        unique_results[label] = f"{label},({int(camera_coordinates[0])},{int(camera_coordinates[1])},{50})"
        destination_list [label] = f"{label},({int(camera_coordinates[0])},{int(camera_coordinates[1])},{50})"
...
def main():
    ...
    while True:
        if state == "capture":
            ...
            #     print("輸入無效，請輸入數字")
            #     continue
            object_choice, destination_choice = selection_ui.run_selection_ui(object_list, destination_list)

            # 打印結果
            print(f"選擇的物件: {object_choice}")
            print(f"選擇的目標: {destination_choice}")
